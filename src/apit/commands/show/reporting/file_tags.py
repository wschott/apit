...
class FileTags:
    ...
    def filter(self, tags_to_select: Iterable[str]) -> list[TagIdDescriptionValue]:
    ...
    def _calculate_selectable_tags(self, tags_to_select: Iterable[str]) -> list[str]:
    ...
    def _find_tag_value_by(self, tag_id: str) -> TagIdDescriptionValue:
        for tag_value in self._all_tag_values:
            if tag_value == tag_id:
                return tag_value
        raise ApitError()

    def get_unknown_tag_ids(self, known_tag_ids: Iterable[str]) -> set[str]:
